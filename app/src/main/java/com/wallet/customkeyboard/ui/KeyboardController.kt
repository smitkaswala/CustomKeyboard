...
abstract class KeyboardController(private val inputConnection: InputConnection)  {
    ...
    companion object {
        ...
        private fun addCharacter(text: String, addition: String, index: Int): String {
    ...
    @Synchronized fun onKeyStroke(c: String) {
        updateMembers()
        handleKeyStroke(c)
    ...
    @Synchronized fun onFontChanged(font: BaseFont) {
        for (listener in listeners) {
            listener.onFontChanged(font)
        }
    }

    @Synchronized fun onNumericChanged(numeric: BaseNumeric) {
        for (listener in listeners) {
            listener.onNumericChanged(numeric)
        }
    }
    ...
    internal fun deletePreviousCharacter() {
        ...
        inputText = deleteCharacter(inputText, --cursorPosition)
        // This is for custom fonts (clears twice)
        //inputConnection.deleteSurroundingText(2, 0)
        //inputText = deleteCharacter(inputText, cursorPosition-2)
    }

    internal fun addCharacter(c: String) {
    ...
    internal fun replaceNextCharacter(c: String) {
        deleteNextCharacter()
        addCharacter(c)
    }
    ...
    internal abstract fun handleKeyStroke(c: String)
